...
| `sw_lan_tproxy` | inet | prerouting | mangle-10 | v4 LAN TCP tproxy + v6 TCP fast-reject + QUIC reject |
...
## Dual-Stack Table (table inet)

The tproxy table uses `table inet` (not `table ip`) because the v6
early-returns (private v6, `cn6_direct`, `cn6_domain_ips`,
`bypass_devices6`, `gmail_smtp6`) and the v6 rejects (TCP reset +
QUIC ICMPv6) must live in the same chain as their v4 counterparts.
Only IPv4 TCP is tproxied: the relay pool is IPv4-only, so IPv6 TCP
is rejected with a TCP reset and clients fall back to the working
v4 path via Happy Eyeballs (instant fallback, measured 2026-09-26:
v6 through the relay never succeeded once).  QUIC (UDP/443) is
rejected (not tproxied) to force HTTP/2 fallback -- QUIC-over-VPN
