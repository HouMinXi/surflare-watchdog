...
if [ "$INIT" = "procd" ]; then
    ...
    # Persist mode to /etc/surflare/mode.conf so scp re-deploys of
    # surflare_watchdog.sh do not reset the mode setting.
    echo "MODE=\"${_deploy_mode}\"" > /etc/surflare/mode.conf
    chmod 644 /etc/surflare/mode.conf
    echo "  mode.conf written (MODE=${_deploy_mode}) -> /etc/surflare/mode.conf"
